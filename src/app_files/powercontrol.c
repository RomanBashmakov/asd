...
void POWER_process()
{
    // в ИЛ-114 используется только первая группа датчиков и всего первые 6 из
    // группы
    ...
    if (slowOnA)
    {
        ...
        if (TIMERS_expired(tmrSlowOnIntervalA))
        {
            ...
            if (slowOnPsNumA < 10)
            {
                if (powerSensA[slowOnPsNumA].connected)
                    T_INA226_AlertOn(&powerSensA[slowOnPsNumA]);
                slowOnPsNumA++;
            }
            else
            {
    ...
    else
    {
        ...
        if (TIMERS_expired(tmrAReadTimeout))
        {
            ...
            // в случае отвала чтения по i2c передергиваем DMA и сбрасываем все
            // состояния
        ...
        if (powerSensA[currentPsNumA].state == ST_INA226_READ_COMPLETE)
        {
            ...
            TIMERS_setTimer(
                &tmrAReadTimeout,
                2000);  // устанавливаем таймер проверки отвала чтения по I2C
            currentPsNumA++;
            if (currentPsNumA > 9)
            {  // в ИЛ-114 обработка только первых 5-ти датчиков в группе А
                currentPsNumA = 0;
                ...
                if (voltagePresent == 0)
                {
                    ...
                    for (int i = 0; i < 10; i++)
                    {
                        if (powerSensA[i].connected)
                            T_INA226_AlertOff(&powerSensA[i]);
                    }
                }
            }
        }
