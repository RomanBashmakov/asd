...
void FLASH_W25Q32_init(SPI_HandleTypeDef *hspi, GPIO_TypeDef *GPIO_PORT_cs,
                       uint16_t GPIO_PIN_cs)
{
    ...
    HAL_GPIO_WritePin(flashW25.GPIO_PORT_cs, flashW25.GPIO_PIN_cs,
                      GPIO_PIN_SET);
    ...
    TTimer tmr;
    TIMERS_setTimer(&tmr, 60000);  // возможно идет стирание всего чипа, поэтому
                                   // таймер таймаута 60 сек
    do
    {
        ...
        if (TIMERS_expired(tmr))
        {  // проверка таймаута, если если бит не установился в течении 60 сек
           // то выходим
...
void FLASH_W25Q32_transmitReceive(uint16_t size)
{
    HAL_GPIO_WritePin(flashW25.GPIO_PORT_cs, flashW25.GPIO_PIN_cs,
                      GPIO_PIN_RESET);
    HAL_SPI_TransmitReceive(flashW25.hspi, flashW25.txBuffer, flashW25.rxBuffer,
                            size, 100);
    HAL_GPIO_WritePin(flashW25.GPIO_PORT_cs, flashW25.GPIO_PIN_cs,
                      GPIO_PIN_SET);
}
...
void FLASH_W25Q32_readData(uint32_t addr, uint8_t *data, uint16_t size)
{
    ...
    if (size > (W25Q32_TXRX_BUFFER_SIZE - 4))
        size = W25Q32_TXRX_BUFFER_SIZE - 4;  // ASSERT
...
// куда записываются данные болжна быть чистой, то есть все байты в области
// должны быть равны 0xFF
void FLASH_W25Q32_writeData(uint32_t addr, uint8_t *data, uint16_t size)
{
    ...
    do
    {
        ...
        if (TIMERS_expired(tmr))
        {  // проверка таймаута, если если бит не установился в течении 1 сек то
           // выходим
            flashW25.timeoutError = 1;
            break;
        }
...
void FLASH_W25Q32_eraseSector(uint32_t sectorAddr)
{
    ...
    do
    {
        ...
        if (TIMERS_expired(tmr))
        {  // проверка таймаута, если если бит не установился в течении 2 сек то
           // выходим
            flashW25.timeoutError = 1;
            break;
        }
...
// происходит очень долго (около минуты) во время этого процесса производить
// чтение/запись в флэш-память нельзя поэтому эту функцию лучше использовать
// перед загрузкой микроконтроллера
void FLASH_W25Q32_eraseChip(void)
{
    ...
    do
    {
        ...
        if (TIMERS_expired(tmr))
        {  // проверка таймаута, если если бит не установился в течении 70 сек
           // то выходим
            flashW25.timeoutError = 1;
            break;
        }
