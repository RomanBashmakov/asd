...
void TERMINAL_init(TTerminal *tns)
{
    if (tns != NULL)
    {
        ...
        CBUFF_init(&tns->cbOutput, tns->outputBuffer,
                   TERM_OUTPUT_BUFFER_LENGTH);
    }
}
...
void TERMINAL_process(TTerminal *tns)
{
    ...
    if (tns->newCmd)
    {
        ...
        {
            strLen = snprintf(strBuffer, TNSERVER_STR_BUFFER_MAXSIZE, strVer,
                              strVersionDT);
        ...
        else if (strncmp(tns->cmd[0], "status", tns->cmdLength[0]) == 0)
        {
            ...
            {
                for (int i = 0; i < 20; i++)
                    tns->cmdStatusModule[i] = tns->cmd[1][i];
                ...
                if (tns->cmdIndex > 1)
                {
                    ...
                    {
                        if (v < 1)
                            v = 1;  // слишком маленький таймер "повесит" IP
                                    // стэк  !!!
        ...
        else if (strncmp(tns->cmd[0], "fan", 3) == 0)
        {
            if (tns->cmdIndex > 0)
            {
                ...
                {
                    HAL_GPIO_WritePin(GPO_blower_GPIO_Port, GPO_blower_Pin,
                                      GPIO_PIN_SET);
...
void TERMINAL_recvByte(TTerminal *tns, char tmp)
{
    ...
    {
        if (tns->cbTerminalCmd[tns->cbTerminalCmdIndex].count ||
            tns->cbTerminalCmdIndex > 0)
        {
            tns->cmdLength[0] = tns->cbTerminalCmd[0].count;
            CBUFF_getFirstNBytes(&tns->cbTerminalCmd[0], tns->cmd[0],
                                 tns->cbTerminalCmd[0].count);

            if (tns->cbTerminalCmdIndex > 0)
            {
                tns->cmdLength[1] = tns->cbTerminalCmd[1].count;
                CBUFF_getFirstNBytes(&tns->cbTerminalCmd[1], tns->cmd[1],
                                     tns->cbTerminalCmd[1].count);
            }
            if (tns->cbTerminalCmdIndex > 1)
            {
                tns->cmdLength[2] = tns->cbTerminalCmd[2].count;
                CBUFF_getFirstNBytes(&tns->cbTerminalCmd[2], tns->cmd[2],
                                     tns->cbTerminalCmd[2].count);
            }
    ...
    {
        if (tmp > (unsigned char)0x20 && tmp <= (unsigned char)0x7E)
        {  // если символ читаемый (в русской кодировке не принимаются)
            // сохранить в буфер команд
            CBUFF_pushNBytes(&tns->cbTerminalCmd[tns->cbTerminalCmdIndex],
                             (char *)&tmp, 1);
        }
        else if ((tmp == (unsigned char)0x20))
        {  // пробел

            // printf ("%d  %d\n",
            // tns->cbTerminalCmd[tns->cbTerminalCmdIndex].count,
...
void TERMINAL_printStatus(TTerminal *tns)
{
    ...
    if (tns)
    {
        if (tns->cmdStatusModule[0] == 0 ||
            strncmp(tns->cmdStatusModule, "stm32", 5) == 0)
        {
            strLen = snprintf(
                strBuffer, TNSERVER_STR_BUFFER_MAXSIZE,
                "stm32:\r\n t1:%.1f  t2:%.1f  vdda:%.3f  debug:%lu  "
                "readProtect:%d  csr:%08lX\r\n uid1: %02X %02X %02X %02X,    "
                "uid2: %02X %02X %02X %02X (%s),    uid3: %02X %02X %02X %02X "
                "(%s)\r\n",
                stm32State.tempLM94022_1, stm32State.tempLM94022_2,
                stm32State.vdda / 1000.0, stm32State.debugEnabled,
                stm32State.readOutEnabled, stm32State.rcccsr,
                stm32State.UID[0][0], stm32State.UID[0][1],
                stm32State.UID[0][2], stm32State.UID[0][3],

                stm32State.UID[1][0], stm32State.UID[1][1],
                stm32State.UID[1][2], stm32State.UID[1][3], stm32State.UID[1],

                stm32State.UID[2][0], stm32State.UID[2][1],
                stm32State.UID[2][2], stm32State.UID[2][3], stm32State.UID[2]);
            TERMINAL_putMsgLen(tns, strBuffer, strLen);
        }

        if (tns->cmdStatusModule[0] == 0 ||
            strncmp(tns->cmdStatusModule, "pwr", 3) == 0)
        {
            char t = 1;

            for (int i = 0; i < 10; i++)
            {
                strLen = snprintf(strBuffer, TNSERVER_STR_BUFFER_MAXSIZE,
                                  "%d: %.1fV %.3fA %.3fW\r\n", t,
        ...
        if (tns->cmdStatusModule[0] == 0 ||
            strncmp(tns->cmdStatusModule, "hub", 3) == 0)
        {
            strLen =
                snprintf(strBuffer, TNSERVER_STR_BUFFER_MAXSIZE, "hubs:\r\n"
        ...
        if (tns->cmdStatusModule[0] == 0 ||
            strncmp(tns->cmdStatusModule, "arinc", 5) == 0)
        {
            strLen = snprintf(
                strBuffer, TNSERVER_STR_BUFFER_MAXSIZE,
                "arinc429:\r\nformat1:%d %d %d, CAPT: coursor x:%d y:%d, "
                "cams:%d joystick:%d%d%d%d\r\nactive: menu:%d cursor:%d "
                "btn:%d%d%d%d%d\r\n",
                arinc429Control.FORMAT1, arinc429Control.FORMAT1_POZITION,
                arinc429Control.FORMAT1_SIZE, arinc429Control.CURSOR_X_CAPT,
                arinc429Control.CURSOR_Y_CAPT, arinc429Control.CAMS_NUMBER_CAPT,
                ...
                arinc429Control.LEFT_ENTER_CAPT, arinc429Control.OK_BTN_CAPT,
                arinc429Control.ESC_BTN_CAPT, arinc429Control.LWR_AREA_BTN_CAPT,
                arinc429Control.RIGHT_ENTER_CAPT);
            TERMINAL_putMsgLen(tns, strBuffer, strLen);

            strLen = snprintf(
                strBuffer, TNSERVER_STR_BUFFER_MAXSIZE,
                "format2:%d %d %d, FO: coursor x:%d y:%d, cams:%d "
                "joystick:%d%d%d%d\r\nactive: menu:%d cursor:%d "
                "btn:%d%d%d%d%d\r\n",
                arinc429Control.FORMAT2, arinc429Control.FORMAT2_POZITION,
                arinc429Control.FORMAT2_SIZE, arinc429Control.CURSOR_X_FO,
                arinc429Control.CURSOR_Y_FO, arinc429Control.CAMS_NUMBER_FO,
                ...
                arinc429Control.ACTIVE_CURSOR_FO, arinc429Control.LEFT_ENTER_FO,
                arinc429Control.OK_BTN_FO, arinc429Control.ESC_BTN_FO,
            ...
            strLen = snprintf(
                strBuffer, TNSERVER_STR_BUFFER_MAXSIZE,
                //                         "DT: %02d.%02d.%d  %02d:%02d:%02d pc:
                //                         ctrl %d, dt %d",
                "DT: %02d.%02d.%d  %02d:%02d:%02d   pc: srvrouter_stat %d",
                arinc429DT.day, arinc429DT.month, arinc429DT.year + 2000,
                arinc429DT.hour, arinc429DT.minute, arinc429DT.sec,
