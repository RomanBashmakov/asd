...
uint8_t txPacketARINC429_1[4] =  {   0300, 0x03, 0x00, 0x60 };  // предустановка отправляемых пакетов
...
void ARINC429_init(SPI_HandleTypeDef *hspi){
  ...
  HAL_GPIO_WritePin(GPO_ar429_rstn_GPIO_Port, GPO_ar429_rstn_Pin, GPIO_PIN_SET);
  HAL_Delay(10);
  HAL_GPIO_WritePin(GPO_ar429_rstn_GPIO_Port, GPO_ar429_rstn_Pin, GPIO_PIN_RESET);
  ...
  //while ( HAL_GPIO_ReadPin(hi.pGPIO->PORT_ready, hi.pGPIO->PIN_ready) == GPIO_PIN_RESET ) { // инвертировано для оптопары
  while ( HAL_GPIO_ReadPin(hi.pGPIO->PORT_ready, hi.pGPIO->PIN_ready) == GPIO_PIN_SET ) {
    printf ("test\n"); //
  ...
  // включение выдачи сигнала INT на пин, при приеме нового сообщения в память МС
  ...
  // включение прерываний по приходу сообщений со всеми метками на всех каналах
  ...
  TIMERS_setTimer(&tmrSendTimeoutCh1, A429_L0300_SEND_PERIOD);  // отправка по первому каналу ARINC в интервале 1с
}
...
//    memset( &arinc429DT, 0, sizeof(T_ARINC429ControlDateTime));  // для ИЛ-114
...
//	  pcDTConnected = 0;
  ...
  uint8_t t = HAL_GPIO_ReadPin(hi.pGPIO->PORT_int, hi.pGPIO->PIN_int);	// опрос вывода прерывания HI-3220 (by ZHuchenkov)
...
//  }//DBG
  ...
  if (t == 0){
	...
	HAL_GPIO_WritePin(hi.pGPIO->PORT_ack, hi.pGPIO->PIN_ack, GPIO_PIN_RESET);
	__ddelayT(6);
    ...
    if ((reg & 0x01) == 0x01) {
      ...
      // read count fifo ch0
      HI3220_writeMAP(&hi, FCV0);	// по адресу FCV0 (0x8068) лежит RX FIFO COUNT 0, для универсальности тут лучше сделать (FCV0 + ch)
...
//    HI3220_transmitDirect (&hi, 0, txPacketARINC429_1, 1);
    ...
    ARINC_Word_300.str.cam2_fault = (T_INA226_getPower(&powerSensA[1]) < 2)? true : false; 		if(ARINC_Word_300.str.cam2_fault) { Non_Critical_Fault = true; cams_N_faults++; }
    ARINC_Word_300.str.cam3_fault = (T_INA226_getPower(&powerSensA[2]) < 2)? true : false; 		if(ARINC_Word_300.str.cam3_fault) { Non_Critical_Fault = true; cams_N_faults++; }
    ARINC_Word_300.str.cam4_fault = (T_INA226_getPower(&powerSensA[3]) < 2)? true : false;		if(ARINC_Word_300.str.cam4_fault) { Non_Critical_Fault = true; cams_N_faults++; }
    ARINC_Word_300.str.cam5_fault = (T_INA226_getPower(&powerSensA[4]) < 2)? true : false; 		if(ARINC_Word_300.str.cam5_fault) { Non_Critical_Fault = true; cams_N_faults++; }
    ARINC_Word_300.str.cam6_fault = (T_INA226_getPower(&powerSensA[5]) < 2)? true : false; 		if(ARINC_Word_300.str.cam6_fault) { Non_Critical_Fault = true; cams_N_faults++; }

    // Проверка статусов оборудования
//    ARINC_Word_300.str.System_Status = SYSTEM_Status.system;
    ...
    if(SYSTEM_Status.srvrouter_stat == 1 || ARINC_Word_300.str.Storage_Status == 1 || cams_N_faults >= CHANNELS_TOTAL) {

    	Critical_Fault = true;
    }

	ARINC_Word_300.str.XAE21_fault = SYSTEM_Status.XAE21_fault;				if(ARINC_Word_300.str.XAE21_fault) { Non_Critical_Fault = true; }
	ARINC_Word_300.str.cam_switch_fault = SYSTEM_Status.cam_switch_fault;	if(ARINC_Word_300.str.cam_switch_fault) { Non_Critical_Fault = true; }


	// определение состояния ВСЕЙ СИСТЕМЫ
	if(Critical_Fault) ARINC_Word_300.str.System_Status = SYS_FAULT;
	else if (Non_Critical_Fault) ARINC_Word_300.str.System_Status = SYS_PARTIALLY_FAULT;
	...
	*outSize = 0;
	...
	if (packet[0] == 0x55)
	{
	    ...
	    else if (size == sizeof(GetStatusQuery) && packet[1] == 0x11)
	    {
            ...
            {
                getStatusResponse.status = (T_INA226_getCurrent(&powerSensA[getStatusQuery.camera]) < CAMERA_CURRENT_THRESHOLD ? 0 : 1);
            }

            memcpy(out, (unsigned char *)&getStatusResponse, sizeof(GetStatusResponse));
    ...
    //			cnt++;//DBG
        ...
        //        pcCtrlConnected = 1;
    //			SYSTEM_Status.srvrouter_stat = SYS_NORMAL_OPERATION;	/* когда сервер будет присылать свой реальный статус, поставить именно его и там, где он будет приниматься (здесь временно - только по факту приема пакета от него) */
    // статус сервера приходит еще и через Linux_Status, решить что важнее по логике
                TIMERS_setTimer(&tmrPcRecvTimeoutCtrl, SVR_TIMEOUT);
